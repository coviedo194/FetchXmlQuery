...
La logica principal vive en [FetchXmlQuery.cs](FetchXmlQuery.cs).
...
```bash
dotnet build FetchXmlQuery.csproj
```
...
- Si el build falla, revisa primero [FetchXmlQuery.cs](FetchXmlQuery.cs) y [FetchXmlQuery.csproj](FetchXmlQuery.csproj).

## 5. Flujo de trabajo recomendado

1. Hacer el cambio minimo necesario.
2. Compilar el proyecto con `dotnet build FetchXmlQuery.csproj`.
3. Revisar errores de compilacion antes de seguir.
4. Mantener la logica principal concentrada en [FetchXmlQuery.cs](FetchXmlQuery.cs).
...
- Usar como Plugin Type la clase [FetchXmlQueryPlugin](FetchXmlQuery.cs).
